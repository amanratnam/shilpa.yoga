...
import { buildOnlineVinyasaContent, buildPersonalGurgaonContent } from "@/content/classes";
...
export function buildYogaClassesGurgaon(config: PricingConfig): ServicePageContent {
  const personal = buildPersonalGurgaonContent(config);
  return {
    hero: {
    ...
    pricing: {
      ...
      plans: personal.pricing.plans,
      note: personal.pricing.note,
    },
    faqs: [
      {
...
export function buildOnlineYogaIndia(config: PricingConfig): ServicePageContent {
  const online = buildOnlineVinyasaContent(config);
  return {
    hero: {
