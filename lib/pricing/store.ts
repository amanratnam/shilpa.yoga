...
async function readPublishedConfig(): Promise<PricingConfig> {
  ...
  // the migration is run), a network blip or a row written by an older shape
  // all fall back to the committed defaults rather than throwing into a page.
  if (error || !data) return defaultPricingConfig;

  const parsed = pricingConfigSchema.safeParse(data.config);
  return parsed.success ? parsed.data : defaultPricingConfig;
}

const readCached = unstable_cache(readPublishedConfig, [PRICING_TAG], {
  tags: [PRICING_TAG],
});
